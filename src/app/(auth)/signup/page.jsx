...
const Signup = () => {
  ...
  const submitHandler = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post(`${process.env.NEXT_PUBLIC_API_URL}/api/auth/register`, signupData, { withCredentials: true });
      ...
      <main className="flex justify-center px-4 sm:px-12 lg:px-20 my-12">
        <div className="w-full sm:w-4/5 lg:w-2/6 lg:min-w-[400px] flex flex-col items-center gap-6 border border-white/10 rounded-lg p-6">
          <div className="flex flex-col items-center gap-2">
            ...
            <p className="text-sm opacity-75">Enter your details to start using Codev</p>
          </div>
          <form
